...
func TestUsersService(t *testing.T) {
	...
	t.Run("returns 200 for known user", func(t *testing.T) {
		resp, err := client.Get(baseURL + "/search?email=bart@thesimpsons.com")
		...
		assert.Equal(t, "bart@thesimpsons.com", user["email"])
		assert.Equal(t, "+1 888 123 1115", user["phoneNumber"])
	})
